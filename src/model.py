...
Model yang tersedia:
  ...
  - xception        → Xception         (~22.9M params)
...
MODEL_REGISTRY = {
    ...
    "xception":        {"timm_name": "xception",         "params": "~22.9M"},
}
...
def get_model(model_name: str, freeze_backbone: bool = True):
    ...
    Args:
        model_name    : nama model dari MODEL_REGISTRY (efficientnet_b0, resnet50, xception)
        freeze_backbone: True = hanya latih head (Phase 1), False = latih semua (Phase 2)
    
    Returns:
        model: PyTorch model yang sudah dimodifikasi untuk binary classification
    """
...
def unfreeze_backbone(model):
    ...
    Args:
        model: PyTorch model yang akan di-unfreeze
    """
    for param in model.parameters():
        param.requires_grad = True
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
...
def _build_head(in_features: int) -> nn.Sequential:
    ...
    Arsitektur: Linear(256) -> BatchNorm -> ReLU -> Dropout(0.5) -> Linear(1)
    
    Args:
        in_features: Jumlah fitur dari backbone
